...
module Transport
  class TestAlgorithms < NetSSHTest
    ...
    def test_constructor_should_build_complete_list_of_algorithms_with_append_all_supported_algorithms
      # The weak algorithms deprecated in 6.0 are no longer supported, so the
      # complete list is identical to the default list.
      assert_equal ed_ec_host_keys + %w[ssh-rsa-cert-v01@openssh.com ssh-rsa-cert-v00@openssh.com ssh-rsa rsa-sha2-256 rsa-sha2-512], algorithms(append_all_supported_algorithms: true)[:host_key]
      assert_equal x25519_kex + ec_kex + %w[diffie-hellman-group-exchange-sha256 diffie-hellman-group14-sha256 diffie-hellman-group14-sha1], algorithms(append_all_supported_algorithms: true)[:kex]
      assert_equal chacha_poly_cipher + %w[aes256-ctr aes192-ctr aes128-ctr aes256-gcm@openssh.com aes128-gcm@openssh.com], algorithms(append_all_supported_algorithms: true)[:encryption]
      assert_equal %w[hmac-sha2-512-etm@openssh.com hmac-sha2-256-etm@openssh.com hmac-sha2-512 hmac-sha2-256 hmac-sha1], algorithms(append_all_supported_algorithms: true)[:hmac]
    ...
    def test_constructor_with_preferred_host_key_type_should_put_preferred_host_key_type_first
      assert_equal %w[ssh-rsa] + ed_ec_host_keys + %w[ssh-rsa-cert-v01@openssh.com ssh-rsa-cert-v00@openssh.com rsa-sha2-256 rsa-sha2-512], algorithms(host_key: "ssh-rsa", append_all_supported_algorithms: true)[:host_key]
    end
    ...
    def test_constructor_with_known_hosts_reporting_known_host_key_should_use_that_host_key_type
      ...
      ).returns([stub("key", ssh_type: "ssh-rsa")])
      assert_equal %w[ssh-rsa] + ed_ec_host_keys + %w[ssh-rsa-cert-v01@openssh.com ssh-rsa-cert-v00@openssh.com rsa-sha2-256 rsa-sha2-512], algorithms[:host_key]
    end
    ...
    def test_constructor_with_unrecognized_host_key_type_should_return_whats_supported
      assert_equal ed_ec_host_keys + %w[ssh-rsa-cert-v01@openssh.com ssh-rsa-cert-v00@openssh.com ssh-rsa rsa-sha2-256 rsa-sha2-512],
    ...
    def test_constructor_with_preferred_kex_should_put_preferred_kex_first
      assert_equal %w[diffie-hellman-group14-sha1] + x25519_kex + ec_kex + %w[diffie-hellman-group-exchange-sha256 diffie-hellman-group14-sha256],
                   algorithms(kex: "diffie-hellman-group14-sha1", append_all_supported_algorithms: true)[:kex]
    end

    def test_constructor_with_unrecognized_kex_should_not_raise_exception
      assert_equal %w[diffie-hellman-group14-sha1] + x25519_kex + ec_kex + %w[diffie-hellman-group-exchange-sha256 diffie-hellman-group14-sha256],
                   algorithms(kex: %w[bogus diffie-hellman-group14-sha1], append_all_supported_algorithms: true)[:kex]
    end

    def test_constructor_with_preferred_kex_supports_additions
      # diffie-hellman-group1-sha1 is no longer supported, so attempting to add
      # it back is a no-op (it is dropped during composition).
      assert_equal x25519_kex + ec_kex + %w[diffie-hellman-group-exchange-sha256 diffie-hellman-group14-sha256 diffie-hellman-group14-sha1],
    ...
    def test_constructor_with_preferred_encryption_should_put_preferred_encryption_first
      assert_equal %w[aes128-ctr] + chacha_poly_cipher + %w[aes256-ctr aes192-ctr aes256-gcm@openssh.com aes128-gcm@openssh.com], algorithms(encryption: "aes128-ctr", append_all_supported_algorithms: true)[:encryption]
    end

    def test_constructor_with_multiple_preferred_encryption_should_put_all_preferred_encryption_first
      assert_equal %w[aes128-ctr aes192-ctr] + chacha_poly_cipher + %w[aes256-ctr aes256-gcm@openssh.com aes128-gcm@openssh.com], algorithms(encryption: %w[aes128-ctr aes192-ctr], append_all_supported_algorithms: true)[:encryption]
    end

    def test_constructor_with_unrecognized_encryption_should_keep_whats_supported
      assert_equal %w[aes128-ctr] + chacha_poly_cipher + %w[aes256-ctr aes192-ctr aes256-gcm@openssh.com aes128-gcm@openssh.com], algorithms(encryption: %w[bogus aes128-ctr], append_all_supported_algorithms: true)[:encryption]
    end

    def test_constructor_with_preferred_encryption_supports_additions
      # The weak ciphers deprecated in 6.0 are no longer supported, so attempting
      # to add one back is a no-op (it is dropped during composition).
      assert_equal chacha_poly_cipher + %w[aes256-ctr aes192-ctr aes128-ctr aes256-gcm@openssh.com aes128-gcm@openssh.com],
    ...
    def test_constructor_with_preferred_encryption_supports_removals_with_wildcard
      assert_equal chacha_poly_cipher + %w[aes256-gcm@openssh.com aes128-gcm@openssh.com],
                   algorithms(encryption: %w[-*-ctr])[:encryption]
    end

    def test_constructor_with_preferred_hmac_should_put_preferred_hmac_first
      assert_equal %w[hmac-sha1 hmac-sha2-512-etm@openssh.com hmac-sha2-256-etm@openssh.com hmac-sha2-512 hmac-sha2-256], algorithms(hmac: "hmac-sha1", append_all_supported_algorithms: true)[:hmac]
    end

    def test_constructor_with_multiple_preferred_hmac_should_put_all_preferred_hmac_first
      assert_equal %w[hmac-sha1 hmac-sha2-256 hmac-sha2-512-etm@openssh.com hmac-sha2-256-etm@openssh.com hmac-sha2-512], algorithms(hmac: %w[hmac-sha1 hmac-sha2-256], append_all_supported_algorithms: true)[:hmac]
    end

    def test_constructor_with_unrecognized_hmac_should_ignore_those
      assert_equal %w[hmac-sha2-512-etm@openssh.com hmac-sha2-256-etm@openssh.com hmac-sha2-512 hmac-sha2-256 hmac-sha1],
                   algorithms(hmac: "unknown hmac-sha1", append_all_supported_algorithms: true)[:hmac]
    end

    def test_constructor_with_preferred_hmac_supports_additions
      # The weak MACs deprecated in 6.0 are no longer supported, so attempting to
      # add one back is a no-op (it is dropped during composition).
      assert_equal %w[hmac-sha2-512-etm@openssh.com hmac-sha2-256-etm@openssh.com hmac-sha2-512 hmac-sha2-256 hmac-sha1],
                   algorithms(hmac: %w[+hmac-md5-96])[:hmac]
    end

    def test_constructor_with_preferred_hmac_supports_removals_with_wildcard
      assert_equal %w[hmac-sha2-512-etm@openssh.com hmac-sha2-256-etm@openssh.com hmac-sha2-512 hmac-sha2-256],
                   algorithms(hmac: %w[-hmac-sha1*])[:hmac]
    end
